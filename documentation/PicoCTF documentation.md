...
To me this seems pretty self-explanatory. I first want to open up the inspector by either right-clicking and clicking inspect element on the page or pressing F12 on your keyboard. One of the first places I looked was to see if there was anything stored in the cookies.

![cookies](cookies.png)

As you can see, there was nothing stored in the cookies. Maybe we can take a look at the HTML code of the page. You can look through the HTML code by going to Network and viewing the `.html` files that are loaded when loading up the website. You can look through elements, but any extensions you have installed can possibly inject themselves into the webpage, adding extra HTML code that is not part of the website itself. Snooping around, the HTML documents look normal to me, except for one page...

![html](html.png)

This stands out among the rest because of this "notify_true" tag with these seemingly random combination of characters and numbers. We can try decoding this using an application called CyberChef, which is kind of the Swiss army knife for cyber. Loading up CyberChef, we need to drag the Magic operation into the Recipe. After that, we will paste that odd-looking jumble of text into the Input box, then in the Output box should be our flag, which we can submit.

![cyberchef](cyberchef_actual.gif)

Now that's really easy, let's delve into something harder.
...
Assuming you already have a PicoCTF account, visit the link here for the challenge: [PicoCTF Challenge](https://play.picoctf.org/practice/challenge/380?difficulty=3&page=1). Make sure you are here!

![things](things.png)

Click the launch instance button then head over to the webshell:

![webshell](signing_in_to_webshell.gif)

Make sure when you are copying stuff to use `ctrl+shift+v` to paste it inside of the webshell.

## Analysis:

Now that we are in, let’s take a look at the files we have. We will do that by running `ls -lsa` and you should see an output similar to this:

![ls](ls.png)

For those who are unfamiliar with Linux, we want to take a look at the `txtreader` file and its permissions assigned to it: `-rwsr-xr-x`. We want to specifically look at the `s` part of it, which is the setuid bit. This means that whatever user runs this program, they run it as root, so think of it as the person that has access to anything and everything inside of the system, while we have limited access. Even though we have this huge leverage with the application, we are still unable to read the flag owned by root. Let’s look into the source code of the application.
...
```
Press ```ctrl+shift+v``` to paste it inside, then press ```ctrl+s``` and finally ```ctrl+x```.

Then run this command:
```
g++ racecondition.cpp -o grabflag
```
This will compile our script into a binary we can run. We will then run:
```
./grabflag
```
